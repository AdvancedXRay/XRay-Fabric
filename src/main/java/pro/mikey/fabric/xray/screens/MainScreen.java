...
public class MainScreen extends AbstractScreen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (BlockGroup group: this.blocks) {
            drawStringWithShadow(matrices, textRenderer, "Hello", this.width / 2 - 40, y, 0xFFFFFF);

            y += textRenderer.fontHeight + 10;
            for (BlockEntry entry : group.getEntries()) {
                drawStringWithShadow(matrices, textRenderer, entry.getName(), this.width / 2 - 40, y, 0xFFFFFF);
